...
struct AddressHasher {
  size_t operator()(const Address& address) const {
    ...
    return string_hasher(address.city) * prime * prime +
           string_hasher(address.street) * prime + int_hasher(address.building);
  }
...
struct PersonHasher {
  size_t operator()(const Person& person) const {
    const size_t prime = 19937;
    return string_hasher(person.name) * prime * prime * prime +
           int_hasher(person.height) * prime * prime +
           double_hasher(person.weight) * prime +
