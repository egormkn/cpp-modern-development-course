...
class AirportCounter {
 private:
  static const uint32_t NUM_AIRPORTS = static_cast<uint32_t>(TAirport::Last_);

 public:
  // конструктор по умолчанию: список элементов пока пуст
  AirportCounter() { data.fill(0); }
  ...
  using Items = array<Item, NUM_AIRPORTS>;
  ...
  Items GetItems() const {
    ...
    for (size_t i = 0; i < NUM_AIRPORTS; ++i) {
      items[i] = {static_cast<TAirport>(i), data[i]};
    }
    return items;
  }

 private:
  array<size_t, NUM_AIRPORTS> data;
...
void TestManyConstructions() {
  ...
  for (int step = 0; step < 100'000'000; ++step) {
    AirportCounter<SmallCountryAirports> counter(begin(airports),
                                                 end(airports));
...
void TestMostPopularAirport() {
  ...
  for (int day = 0; day < days_to_explore; ++day) {
    ...
    most_popular[day] =
        max_element(begin(items), end(items), [](auto lhs, auto rhs) {
          return lhs.second < rhs.second;
        })->first;
  }

  ASSERT(all_of(
      begin(most_popular), end(most_popular),
      [&](SmallCountryAirports a) { return a == most_popular.front(); }));
}
