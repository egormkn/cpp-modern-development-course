...
private:
  ...
  static typename Visitor::ResultType TraverseWithQuery(const std::unique_ptr<Node>& node, IndexSegment query_segment, Visitor visitor) {
  ...
  public:
    ...
    Data ProcessEmpty(const std::unique_ptr<Node>& node) const {
      return {};
    }

    Data ProcessFull(const std::unique_ptr<Node>& node) const {
      return node->data;
    }

    Data ProcessPartial(const std::unique_ptr<Node>&, IndexSegment, const Data& left_result, const Data& right_result) const {
  ...
  public:
    ...
    void ProcessEmpty(const std::unique_ptr<Node>&) const {}

    void ProcessFull(const std::unique_ptr<Node>& node) const {
      node->postponed_bulk_operation.CombineWith(operation_);
      node->data = operation_.Collapse(node->data, node->segment);
    }

    void ProcessPartial(const std::unique_ptr<Node>& node, IndexSegment) const {
  ...
  static void PropagateBulkOperation(const std::unique_ptr<Node>& node) {
